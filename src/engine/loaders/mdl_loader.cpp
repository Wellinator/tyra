...
MeshFrame *MDLoader::load_md3(u32 &o_framesCount, char *t_subpath, char *t_nameWithoutExtension, float t_scale, u8 t_invertT)
{
    ...
    assertMsg((header.ident == MD3_IDENT) && (header.version == MD3_VERSION), "This MD3 file was not in correct format!");

    u32 framesCount = header.numFrames;
    u32 vertexCount = normal.xyz[3];
    u32 stsCount = str.st[2];
    u32 trianglesCount = surface.numTriangles;

    char framesBuffer[framesCount * surface.ofsSt];
    fseek(file, header.ofsFrames, SEEK_SET);
    fread(&framesBuffer, framesCount * surface.ofsSt, 1, file);

    char stsBuffer[stsCount * sizeof(texCoord_t)];
    fseek(file, surface.ofsSt, SEEK_SET);
    fread(&stsBuffer, stsCount * sizeof(texCoord_t), 1, file);

    char trianglesBuffer[trianglesCount * sizeof(triangle_t)];
    fseek(file, surface.ofsTriangles, SEEK_SET);
    fread(&trianglesBuffer, trianglesCount * sizeof(triangle_t), 1, file);
    ...
    for (u32 j = 0; j < framesCount; j++)
    {
        ...
        resultFrames[j].getMaterial(0).allocateFaces(trianglesCount * 3);
        resultFrames[j].getMaterial(0).setName(t_nameWithoutExtension);
        frame = (frame_t *)&framesBuffer[surface.numFrames * j];
        for (u32 i = 0; i < vertexCount; i++)
        {
            tempVec.set(
                ((frame->verts[i].v[0] * frame->scale[0]) + frame->translate[0]) * t_scale,
    ...
    for (u32 i = 0; i < stsCount; i++)
    {
        texCoord = (texCoord_t *)&stsBuffer[sizeof(texCoord_t) * i];
        tempPoint.set(
            (float)texCoord->s / header.numSkins,
            (float)texCoord->t / header.numSkins);
        if (t_invertT)
            tempPoint.y = 1.0F - tempPoint.y;
        for (u32 j = 0; j < framesCount; j++)
            resultFrames[j].setST(i, tempPoint);
